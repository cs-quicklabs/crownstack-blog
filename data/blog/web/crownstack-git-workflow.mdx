---
title: "Crownstack's official Git workflow"
date: '2023-04-15'
lastmod: '2023-04-15'
...
![Git workflow at Crownstack](/static/images/blogs/web/crownstack-git-workflow/git-workflow.png)
...
### Setting up primary branches for all the application environments
...
Once branching out a feature branch from `main` or `master`, developers can now develop new code in this branch, then merge it back to primary branches one by one. Starting with `development` branch all the way up to `main` or `master`, while merging new branch's code in every primary branch, the testing and approvals by relevant testers is required to move code to next level.
